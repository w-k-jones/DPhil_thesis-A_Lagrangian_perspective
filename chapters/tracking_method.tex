...
\begin{tabular}{lrrr}
\tophline
Instrument   & wavelength   & NE$\Delta$T [300\,K]    & NE$\Delta$T [220\,K] \\
\middlehline
...
\middlehline
SEVIRI (MSG--11)       & 6.2\,\unit{\mu m} & 0.05\,K & 0.448\,K\\
             & 7.3\,\unit{\mu m} & 0.05\,K & 0.294\,K\\
             & 10.8\,\unit{\mu m} & 0.06\,K & 0.165\,K\\
             & 12.0\,\unit{\mu m} & 0.10\,K & 0.237\,K\\
\bottomhline
\end{tabular}
\caption[
Operating noise equivalent $\Delta$T (NE$\Delta$T) for the \acrshort{abi} and \acrshort{seviri} instruments. 
]{
Operating noise equivalent $\Delta$T (NE$\Delta$T) for the \acrshort{abi} and \acrshort{seviri} instruments. NE$\Delta$T is shown both at 300\,K and at 220\,K. \acrshort{abi} data is from {https://www.star.nesdis.noaa.gov/GOESCal/G16\_ABI\_INST\_CAL\_daily\_allmode.php}; \acrshort{seviri} data is from \citet{pili_inorbit_2016}.
} % Table Footnotes
...
The NE$\Delta$T values are provided at a reference temperature of 300\,K.
However, as NE$\Delta$T varies with temperature, it is also important to consider the noise at the colder temperatures typical of anvil clouds.
NE$\Delta$T at 220\,K were calculated by multiplying the NE$\Delta$T at 300\,K by the ratio of the partial derivative of the Planck function at 300\,K to that at 220\,K.
This results in a noticeably larger NE$\Delta$T at the lower temperature, in particular for the \acrshort{wv} channels due to the difference in the gradient of the Planck function between the two temperatures.
Propagating these uncertainties provides estimates of the noise in the \acrshort{wvd} and \acrshort{swd} combinations of 0.180\,K and 0.097\,K respectively for \acrshort{abi}, and 0.536\,K and 0.289\,K for \acrshort{seviri}.

In addition to the NE$\Delta$T, there are several sources of systematic errors which may affect the observed \acrshort{bt} and channel differences.
...
Above 3\,km, the 10.4\,\unit{\mu m} shows a consistent cooling of 0.5\,\unit{K minute^{-1}}.
...
Using the NE$\Delta$T values provided in table~\ref{table:channel_nedt}, the uncertainty due to sensor noise in the \acrshort{bt} difference over a five--minute period is approximately 0.047\,\unit{K minute^{-1}}, or just less than 10\% of the threshold given by \citet{roberts_nowcasting_2003}.
By averaging the cooling rates over 5 pixels and a 15--minute time period the uncertainty is reduced to 0.012\,\unit{K minute^{-1}}, or less than 2.5\%.
...
Taking 6\,\unit{K/km} as a typical value for the moist pseudo--adiabatic lapse rate, the cooling rate for a convective cloud top rising at 1\,\unit{ms^{-1}} is 0.36\,\unit{K minute^{-1}}.
...
As a result, the actual error in the \acrshort{bt} difference over time due to horizontal cloud motions not captured by the optical flow algorithm is likely much smaller, on a similar magnitude to the NE$\Delta$T.
